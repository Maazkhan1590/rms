...
class EditorialAppointmentController extends Controller
{
    ...
    private function getDataTableData(Request $request)
    {
        ...
        $data = $appointments->map(function($item) {
            ...
            $actions .= '<a href="' . route('admin.editorial-appointments.show', $item->id) . '" class="btn btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a>';
            
            if (in_array($item->status, ['pending_coordinator', 'pending_dean']) && $workflow && $workflow->assigned_to == auth()->id()) {
                $actions .= '<form action="' . route('admin.editorial-appointments.approve', $item->id) . '" method="POST" style="display:inline;" onsubmit="return confirm(\'Are you sure you want to approve this editorial appointment?\');">';
                $actions .= csrf_field();
    ...
    public function show(EditorialAppointment $appointment)
    {
        ...
        $appointment->load(['submitter', 'user', 'approver', 'evidenceFiles', 'workflow']);
