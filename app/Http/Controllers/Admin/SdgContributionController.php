...
class SdgContributionController extends Controller
{
    ...
    private function getDataTableData(Request $request)
    {
        ...
        $data = $contributions->map(function($item) {
            ...
            $actions .= '<a href="' . route('admin.sdg-contributions.show', $item->id) . '" class="btn btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a>';
            
            if (in_array($item->status, ['pending_coordinator', 'pending_dean']) && $workflow && $workflow->assigned_to == auth()->id()) {
                $actions .= '<form action="' . route('admin.sdg-contributions.approve', $item->id) . '" method="POST" style="display:inline;" onsubmit="return confirm(\'Are you sure you want to approve this SDG contribution?\');">';
                $actions .= csrf_field();
    ...
    public function show(SdgContribution $contribution)
    {
        ...
        $contribution->load(['submitter', 'user', 'approver', 'evidenceFiles', 'workflow']);
