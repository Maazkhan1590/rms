...
class ConferenceActivityController extends Controller
{
    ...
    private function getDataTableData(Request $request)
    {
        ...
        $data = $activities->map(function($item) {
            ...
            $actions .= '<a href="' . route('admin.conference-activities.show', $item->id) . '" class="btn btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a>';
            
            if (in_array($item->status, ['pending_coordinator', 'pending_dean']) && $workflow && $workflow->assigned_to == auth()->id()) {
                $actions .= '<form action="' . route('admin.conference-activities.approve', $item->id) . '" method="POST" style="display:inline;" onsubmit="return confirm(\'Are you sure you want to approve this conference activity?\');">';
                $actions .= csrf_field();
    ...
    public function show(ConferenceActivity $activity)
    {
        ...
        $activity->load(['submitter', 'user', 'approver', 'evidenceFiles', 'workflow']);
