...
class InternalFundingController extends Controller
{
    ...
    private function getDataTableData(Request $request)
    {
        ...
        $data = $fundings->map(function($item) {
            ...
            $actions .= '<a href="' . route('admin.internal-fundings.show', $item->id) . '" class="btn btn-sm btn-info" title="View"><i class="fa fa-eye"></i></a>';
            
            if (in_array($item->status, ['pending_coordinator', 'pending_dean']) && $workflow && $workflow->assigned_to == auth()->id()) {
                $actions .= '<form action="' . route('admin.internal-fundings.approve', $item->id) . '" method="POST" style="display:inline;" onsubmit="return confirm(\'Are you sure you want to approve this internal funding?\');">';
                $actions .= csrf_field();
