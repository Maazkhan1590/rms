...
class InternalFundingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $years = InternalFunding::distinct()->pluck('year')->filter()->sortDesc()->values();
        $users = User::whereHas('internalFundings')->pluck('name', 'id');

        return view('admin.internal-fundings.index', compact('statuses', 'years', 'users'));
    }

    private function getDataTableData(Request $request)
    {
        $query = InternalFunding::with(['submitter', 'user', 'approver', 'workflow']);
        ...
        if ($orderBy === 'submitted_by') {
            $query->leftJoin('users', 'internal_fundings.submitted_by', '=', 'users.id')
                  ->orderBy('users.name', $orderDir)
                  ->select('internal_fundings.*');
        } else {
        ...
        $data = $fundings->map(function($item) {
            ...
            $actions .= '<a class="btn btn-sm btn-outline-primary" href="' . route('admin.internal-fundings.show', $item->id) . '" title="View" aria-label="View"><span class="material-icons-outlined">visibility</span></a>';
            ...
            if ($canShowActions) {
                $actions .= '<form action="' . route('admin.internal-fundings.approve', $item->id) . '" method="POST" style="display: inline;" class="approve-submission-form">';
                $actions .= csrf_field();
                ...
                $actions .= '</form>';
                $actions .= '<button type="button" class="btn btn-sm btn-outline-danger btn-reject-submission" title="Reject" aria-label="Reject" data-reject-url="' . route('admin.internal-fundings.reject', $item->id) . '"><span class="material-icons-outlined">cancel</span></button>';
            }
        ...
        return response()->json([
            ...
            'recordsTotal' => InternalFunding::count(),
            'recordsFiltered' => $totalRecords,
            'data' => $data,
        ]);
    }

    public function show(InternalFunding $funding)
    {
        ...
        $evidenceFiles = \App\Models\EvidenceFile::where('submission_type', 'internal_funding')
            ->where('submission_id', $funding->id)
            ->with('uploader')
            ->get();
        ...
        return view('admin.internal-fundings.show', compact('funding'));
    }

    public function approve(InternalFunding $funding)
    {
        ...
        try {
            ...
            if (!$workflow) {
                $workflow = $this->workflowService->createWorkflow('internal_funding', $funding->id, $funding->submitter ?? auth()->user());
            ...
            if (!$canApprove) {
                ...
                return redirect()->back()
                    ->with('error', 'You are not authorized to approve this internal funding. Only the currently assigned approver can approve at the current workflow step.');
            }
            ...
            $this->loggingService->logActivity(
                'internal_funding_approved',
                "Approved internal funding: {$funding->project_title}",
                InternalFunding::class,
                $funding->id
            );

            return redirect()->route('admin.internal-fundings.show', $funding)
                ->with('success', 'Internal funding approved successfully!');
        } catch (\Exception $e) {
            \DB::rollBack();
            return redirect()->back()
                ->with('error', 'Error approving internal funding: ' . $e->getMessage());
        }
    }

    public function reject(Request $request, InternalFunding $funding)
    {
        abort_if(Gate::denies('publication_approve'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        ...
        try {
            ...
            $this->loggingService->logActivity(
                'internal_funding_rejected',
                "Rejected internal funding: {$funding->project_title}",
                InternalFunding::class,
                $funding->id
            );

            return redirect()->route('admin.internal-fundings.index')
                ->with('success', 'Internal funding rejected successfully.');
        } catch (\Exception $e) {
            \DB::rollBack();
            return redirect()->back()
                ->with('error', 'Error rejecting internal funding: ' . $e->getMessage());
        }
    }

    public function destroy(InternalFunding $funding)
    {
        ...
        return redirect()->route('admin.internal-fundings.index')
            ->with('success', 'Internal funding deleted successfully.');
    }
}
