...
return new class extends Migration
{
    ...
    {
        Schema::table('partnerships_mous', function (Blueprint $table) {
            $table->enum('status', ['draft', 'submitted', 'pending_coordinator', 'pending_dean', 'approved', 'rejected', 'returned', 'active', 'inactive', 'expired'])->default('draft')->change();
            $table->foreignId('submitted_by')->nullable()->after('lead_staff_id')->constrained('users')->nullOnDelete();
            ...
            $table->year('year')->nullable()->after('evidence_description');
    ...
    public function down(): void
    {
        Schema::table('partnerships_mous', function (Blueprint $table) {
            ...
            $table->enum('status', ['active', 'inactive', 'expired'])->default('active')->change();
        });
    }
};
