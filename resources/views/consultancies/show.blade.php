...
@section('title', $consultancy->project_consultancy_name . ' | Consultancy/KT Details - RMS')
...
<section class="publication-detail-page">
    <div class="container">
        <div class="publication-detail-card">
            <a href="{{ route('consultancies.index') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; color: #6b7280; text-decoration: none; margin-bottom: 1.5rem; font-weight: 500;">
                <i class="fas fa-arrow-left"></i> Back to Consultancies
            </a>

            @auth
                @if($consultancy->status === 'draft' && ($consultancy->submitted_by === auth()->id()))
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        ...
                        <form action="{{ route('consultancies.submit', $consultancy->id) }}" method="POST" style="margin: 0;">
                            @csrf
            ...
            <div class="publication-header">
                <h1 class="publication-title">{{ $consultancy->project_consultancy_name }}</h1>
                @if($consultancy->submitter)
                <div style="margin-bottom: 1rem;">
                    <strong>Submitted by:</strong> {{ $consultancy->submitter->name }}
                </div>
                @endif
            
                @php
                    $displayStatus = $consultancy->status;
                    $statusColors = [
            ...
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-info-circle" style="color: #3b82f6; margin-right: 0.5rem;"></i>Consultancy Details
                </h2>
                <div class="detail-grid">
                    @if($consultancy->client_sponsor)
                    <div class="detail-item">
                        <div class="detail-label">Client/Sponsor</div>
                        <div class="detail-value">{{ $consultancy->client_sponsor }}</div>
                    </div>
                    @endif
                    @if($consultancy->start_date)
                    <div class="detail-item">
                        <div class="detail-label">Start Date</div>
                        <div class="detail-value">{{ $consultancy->start_date->format('F d, Y') }}</div>
                    </div>
                    @endif
                    @if($consultancy->end_date)
                    <div class="detail-item">
                        <div class="detail-label">End Date</div>
                        <div class="detail-value">{{ $consultancy->end_date->format('F d, Y') }}</div>
                    </div>
                    @endif
                    @if($consultancy->amount_omr)
                    <div class="detail-item">
                        <div class="detail-label">Amount (OMR)</div>
                        <div class="detail-value">{{ number_format($consultancy->amount_omr, 2) }}</div>
                    </div>
                    @endif
                    @if($consultancy->income_type)
                    <div class="detail-item">
                        <div class="detail-label">Income Type</div>
                        <div class="detail-value">{{ ucfirst($consultancy->income_type) }}</div>
                    </div>
                    @endif
                    @if($consultancy->points_allocated)
                    <div class="detail-item">
                        <div class="detail-label">Points Allocated</div>
                        <div class="detail-value" style="font-weight: 700; color: #059669;">{{ number_format($consultancy->points_allocated, 2) }}</div>
                    </div>
                    @endif
                </div>
            </div>
            ...
            @if($consultancy->submitter || $consultancy->approver || isset($consultancy->faculty))
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value">{{ $consultancy->submitter->name }}</div>
                        @if($consultancy->submitted_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $consultancy->submitted_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if($consultancy->approver)
                    <div class="detail-item">
                        ...
                        @endif
                        <div class="detail-value">{{ $consultancy->approver->name }}</div>
                        @if($consultancy->approved_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $consultancy->approved_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if(isset($consultancy->faculty) && $consultancy->faculty)
                    <div class="detail-item">
                        <div class="detail-label"><i class="fas fa-university" style="margin-right: 0.5rem;"></i>Faculty</div>
                        <div class="detail-value">{{ $consultancy->faculty }}</div>
                    </div>
                    @endif
                </div>
            </div>
