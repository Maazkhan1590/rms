...
@section('title', 'Consultancies & KT | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>Consultancies & KT</h1>
        <p>Browse approved consultancies and knowledge transfer activities</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('consultancies.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search consultancies..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="type" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach(['consultancy', 'service', 'product'] as $type)
                ...
                <select name="year" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($consultancies->pluck('year')->unique()->filter()->sortDesc() as $year)
...
<section class="publications-grid-section">
    <div class="container">
        ...
        <div class="publications-grid-full" id="consultancies-container">
            @include('consultancies.partials.consultancy-card', ['consultancies' => $consultancies])
        </div>
        @if($hasMore ?? false)
        <div style="text-align: center; margin-top: 3rem;">
            <button id="load-more-btn" class="btn btn-primary" style="padding: 1rem 3rem; font-size: 1.1rem;">
                <i class="fas fa-arrow-down"></i> Load More Consultancies
            </button>
            <div id="loading-indicator" style="display: none; margin-top: 1rem;">
                ...
                <p style="margin-top: 0.5rem; color: var(--text-light);">Loading more consultancies...</p>
            </div>
        </div>
        @endif
        @else
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            ...
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No consultancies available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const consultanciesContainer = document.getElementById('consultancies-container');
    let currentOffset = {{ $consultancies->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            fetch('{{ route("consultancies.load-more") }}?' + new URLSearchParams(params), {
                headers: {
            ...
            .then(data => {
                if (data.html) {
                    ...
                    cards.forEach(card => {
                        consultanciesContainer.appendChild(card);
                    });
                    ...
                    if (data.hasMore) {
                        ...
                        allLoadedMsg.innerHTML = '<i class="fas fa-check-circle"></i> All consultancies loaded';
                        loadingIndicator.parentElement.appendChild(allLoadedMsg);
                    }
                }
            })
            .catch(error => {
                console.error('Error loading more consultancies:', error);
                loadMoreBtn.style.display = 'block';
