...
@section('title', $supervision->student_name . ' | Supervision/Exam Details - RMS')
...
<section class="publication-detail-page">
    <div class="container">
        <div class="publication-detail-card">
            <a href="{{ route('supervision-exams.index') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; color: #6b7280; text-decoration: none; margin-bottom: 1.5rem; font-weight: 500;">
                <i class="fas fa-arrow-left"></i> Back to Supervisions & Exams
            </a>

            @auth
                @if($supervision->workflow_status === 'draft' && ($supervision->submitted_by === auth()->id()))
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        ...
                        <form action="{{ route('supervision-exams.submit', $supervision->id) }}" method="POST" style="margin: 0;">
                            @csrf
            ...
            <div class="publication-header">
                <h1 class="publication-title">{{ $supervision->student_name }}</h1>
                @if($supervision->submitter)
                <div style="margin-bottom: 1rem;">
                    <strong>Submitted by:</strong> {{ $supervision->submitter->name }}
                </div>
                @endif
            
                @php
                    $displayStatus = ($supervision->workflow_status ?? $supervision->status);
                    $statusColors = [
            ...
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-info-circle" style="color: #3b82f6; margin-right: 0.5rem;"></i>Supervision Details
                </h2>
                <div class="detail-grid">
                    @if($supervision->role)
                    <div class="detail-item">
                        <div class="detail-label">Role</div>
                        <div class="detail-value">{{ $supervision->role }}</div>
                    </div>
                    @endif
                    @if($supervision->degree)
                    <div class="detail-item">
                        <div class="detail-label">Degree</div>
                        <div class="detail-value">{{ $supervision->degree }}</div>
                    </div>
                    @endif
                    @if($supervision->university)
                    <div class="detail-item">
                        <div class="detail-label">University</div>
                        <div class="detail-value">{{ $supervision->university }}</div>
                    </div>
                    @endif
                    @if($supervision->status)
                    <div class="detail-item">
                        <div class="detail-label">Status</div>
                        <div class="detail-value">{{ $supervision->status }}</div>
                    </div>
                    @endif
                    @if($supervision->start_year)
                    <div class="detail-item">
                        <div class="detail-label">Start Year</div>
                        <div class="detail-value">{{ $supervision->start_year }}@if($supervision->end_year) - {{ $supervision->end_year }}@endif</div>
                    </div>
                    @endif
                    @if($supervision->academic_year)
                    <div class="detail-item">
                        <div class="detail-label">Academic Year</div>
                        <div class="detail-value">{{ $supervision->academic_year }}</div>
                    </div>
                    @endif
                    @if($supervision->points_allocated)
                    <div class="detail-item">
                        <div class="detail-label">Points Allocated</div>
                        <div class="detail-value" style="font-weight: 700; color: #059669;">{{ number_format($supervision->points_allocated, 2) }}</div>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-book" style="color: #3b82f6; margin-right: 0.5rem;"></i>Thesis Title
                </h2>
            ...
            @if($supervision->evidence_link || $supervision->evidenceFiles->count() > 0 || $supervision->evidence_description)
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-paperclip" style="color: #3b82f6; margin-right: 0.5rem;"></i>Evidence & Attachments
                </h2>
                
                @if($supervision->evidence_description)
                <div style="margin-bottom: 1.5rem; padding: 1rem; background: #f9fafb; border-radius: 8px;">
                    ...
                    <p style="color: #4b5563; margin: 0;">{{ $supervision->evidence_description }}</p>
                </div>
                @endif

                @if($supervision->evidence_link)
                <div style="margin-bottom: 1.5rem;">
                    <strong style="color: #374151; display: block; margin-bottom: 0.5rem;">Evidence Link:</strong>
                    <a href="{{ $supervision->evidence_link }}" target="_blank" style="color: #3b82f6; text-decoration: none; word-break: break-all;">
                        <i class="fas fa-external-link-alt"></i> {{ $supervision->evidence_link }}
                    </a>
                </div>
                @endif
            ...
            @if($supervision->submitter || $supervision->approver || isset($supervision->faculty))
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value">{{ $supervision->submitter->name }}</div>
                        @if($supervision->submitted_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $supervision->submitted_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if($supervision->approver)
                    <div class="detail-item">
                        @if($supervision->status === 'rejected')
                        ...
                        @endif
                        <div class="detail-value">{{ $supervision->approver->name }}</div>
                        @if($supervision->approved_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $supervision->approved_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if(isset($supervision->faculty) && $supervision->faculty)
                    <div class="detail-item">
                        <div class="detail-label"><i class="fas fa-university" style="margin-right: 0.5rem;"></i>Faculty</div>
                        <div class="detail-value">{{ $supervision->faculty }}</div>
                    </div>
                    @endif
                </div>
            </div>
