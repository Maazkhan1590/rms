...
@section('title', 'Research Fellows | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>Research Fellows</h1>
        <p>Browse approved research fellow publications</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('research-fellows.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search research fellows..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="year" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($fellows->pluck('year')->unique()->filter()->sortDesc() as $year)
...
<section class="publications-grid-section">
    <div class="container">
        @if($fellows->count() > 0)
        <div class="publications-grid-full" id="fellows-container">
            @include('research-fellows.partials.fellow-card', ['fellows' => $fellows])
        </div>
        @if($hasMore ?? false)
        ...
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            ...
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No research fellow records available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const fellowsContainer = document.getElementById('fellows-container');
    let currentOffset = {{ $fellows->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            fetch('{{ route("research-fellows.load-more") }}?' + new URLSearchParams(params), {
                headers: {
            ...
            .then(data => {
                if (data.html) {
                    ...
                    cards.forEach(card => {
                        fellowsContainer.appendChild(card);
                    });
