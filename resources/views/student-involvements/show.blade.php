...
@section('title', ucfirst($involvement->category) . ' | Student Involvement Details - RMS')
...
<section class="publication-detail-page">
    <div class="container">
        <div class="publication-detail-card">
            <a href="{{ route('student-involvements.index') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; color: #6b7280; text-decoration: none; margin-bottom: 1.5rem; font-weight: 500;">
                <i class="fas fa-arrow-left"></i> Back to Student Involvements
            </a>

            @auth
                @if($involvement->status === 'draft' && ($involvement->submitted_by === auth()->id()))
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        ...
                        <form action="{{ route('student-involvements.submit', $involvement->id) }}" method="POST" style="margin: 0;">
                            @csrf
            ...
            <div class="publication-header">
                <h1 class="publication-title">{{ ucfirst($involvement->category) }}</h1>
                @if($involvement->submitter)
                <div style="margin-bottom: 1rem;">
                    <strong>Submitted by:</strong> {{ $involvement->submitter->name }}
                </div>
                @endif
            
                @php
                    $displayStatus = $involvement->status;
                    $statusColors = [
            ...
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-info-circle" style="color: #3b82f6; margin-right: 0.5rem;"></i>Involvement Details
                </h2>
                <div class="detail-grid">
                    <div class="detail-item">
                        <div class="detail-label">Count</div>
                        <div class="detail-value">{{ $involvement->count }} student(s)</div>
                    </div>
                    @if($involvement->date)
                    <div class="detail-item">
                        <div class="detail-label">Date</div>
                        <div class="detail-value">{{ $involvement->date->format('F d, Y') }}</div>
                    </div>
                    @endif
                    @if($involvement->academic_year)
                    <div class="detail-item">
                        <div class="detail-label">Academic Year</div>
                        <div class="detail-value">{{ $involvement->academic_year }}</div>
                    </div>
                    @endif
                    @if($involvement->points_allocated)
                    <div class="detail-item">
                        <div class="detail-label">Points Allocated</div>
                        <div class="detail-value" style="font-weight: 700; color: #059669;">{{ number_format($involvement->points_allocated, 2) }}</div>
                    </div>
                    @endif
                </div>
            </div>

            @if($involvement->notes)
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
            ...
            @if($involvement->evidence_link || $involvement->evidenceFiles->count() > 0 || $involvement->evidence_description)
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-paperclip" style="color: #3b82f6; margin-right: 0.5rem;"></i>Evidence & Attachments
                </h2>
                
                @if($involvement->evidence_description)
                <div style="margin-bottom: 1.5rem; padding: 1rem; background: #f9fafb; border-radius: 8px;">
                    ...
                    <p style="color: #4b5563; margin: 0;">{{ $involvement->evidence_description }}</p>
                </div>
                @endif

                @if($involvement->evidence_link)
                <div style="margin-bottom: 1.5rem;">
                    <strong style="color: #374151; display: block; margin-bottom: 0.5rem;">Evidence Link:</strong>
                    <a href="{{ $involvement->evidence_link }}" target="_blank" style="color: #3b82f6; text-decoration: none; word-break: break-all;">
                        <i class="fas fa-external-link-alt"></i> {{ $involvement->evidence_link }}
                    </a>
                </div>
                @endif

                @if($involvement->evidenceFiles && $involvement->evidenceFiles->count() > 0)
                <table class="evidence-table">
                    ...
                    <tbody>
                        @foreach($involvement->evidenceFiles as $file)
                        <tr>
            ...
            @if($involvement->submitter || $involvement->approver || isset($involvement->faculty))
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value">{{ $involvement->submitter->name }}</div>
                        @if($involvement->submitted_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $involvement->submitted_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if($involvement->approver)
                    <div class="detail-item">
                        @if($involvement->status === 'rejected')
                        ...
                        @endif
                        <div class="detail-value">{{ $involvement->approver->name }}</div>
                        @if($involvement->approved_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $involvement->approved_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if(isset($involvement->faculty) && $involvement->faculty)
                    <div class="detail-item">
                        <div class="detail-label"><i class="fas fa-university" style="margin-right: 0.5rem;"></i>Faculty</div>
                        <div class="detail-value">{{ $involvement->faculty }}</div>
                    </div>
                    @endif
                </div>
            </div>
