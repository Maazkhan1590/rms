...
@section('title', 'Student Involvements | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>Student Involvements</h1>
        <p>Browse approved student involvement records</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('student-involvements.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search student involvements..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="category" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    <option value="">All Categories</option>
                    @foreach($involvements->pluck('category')->unique()->filter() as $category)
                ...
                <select name="sort" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    <option value="category" {{ request('sort') == 'category' ? 'selected' : '' }}>Category A-Z</option>
                </select>
...
<section class="publications-grid-section">
    <div class="container">
        ...
        <div class="publications-grid-full" id="involvements-container">
            @include('student-involvements.partials.involvement-card', ['involvements' => $involvements])
        </div>
        @if($hasMore ?? false)
        ...
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            ...
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No student involvement records available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const involvementsContainer = document.getElementById('involvements-container');
    let currentOffset = {{ $involvements->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            fetch('{{ route("student-involvements.load-more") }}?' + new URLSearchParams(params), {
                headers: {
            ...
            .then(data => {
                if (data.html) {
                    ...
                    cards.forEach(card => {
                        involvementsContainer.appendChild(card);
                    });
