...
@section('title', 'SDG Contributions | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>SDG Contributions</h1>
        <p>Browse approved SDG contributions</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('sdg-contributions.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search SDG contributions..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="sdg" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    <option value="">All SDGs</option>
                    @for($i = 1; $i <= 17; $i++)
                        <option value="{{ $i }}" {{ request('sdg') == $i ? 'selected' : '' }}>
                            SDG {{ $i }}
                        </option>
                    @endfor
                </select>
                <select name="type" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($contributions->pluck('type')->unique()->filter() as $type)
                ...
                <select name="year" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($contributions->pluck('year')->unique()->filter()->sortDesc() as $year)
...
<section class="publications-grid-section">
    <div class="container">
        ...
        <div class="publications-grid-full" id="contributions-container">
            @include('sdg-contributions.partials.contribution-card', ['contributions' => $contributions])
        </div>
        @if($hasMore ?? false)
        <div style="text-align: center; margin-top: 3rem;">
            <button id="load-more-btn" class="btn btn-primary" style="padding: 1rem 3rem; font-size: 1.1rem;">
                <i class="fas fa-arrow-down"></i> Load More Contributions
            </button>
            <div id="loading-indicator" style="display: none; margin-top: 1rem;">
                ...
                <p style="margin-top: 0.5rem; color: var(--text-light);">Loading more contributions...</p>
            </div>
        </div>
        @endif
        @else
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            <i class="fas fa-globe" style="font-size: 3rem; color: var(--text-secondary); margin-bottom: 1rem;"></i>
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No SDG contributions available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentOffset = {{ $contributions->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            const params = {
                ...
                sdg: urlParams.get('sdg') || '',
                type: urlParams.get('type') || '',
                year: urlParams.get('year') || '',
                sort: urlParams.get('sort') || 'newest',
            };

            fetch('{{ route("sdg-contributions.load-more") }}?' + new URLSearchParams(params), {
                headers: {
            ...
            .then(data => {
                if (data.html) {
                    ...
                    if (data.hasMore) {
                        ...
                        allLoadedMsg.innerHTML = '<i class="fas fa-check-circle"></i> All contributions loaded';
