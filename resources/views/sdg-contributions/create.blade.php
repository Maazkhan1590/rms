...
@section('title', 'Submit SDG Contribution | Academic Research Portal')
    ...
    <div class="container">
        <div class="auth-container">
            <div class="auth-header">
                <h1>Submit SDG Contribution</h1>
                <p>Fill in the details below to submit your SDG contribution for approval</p>
            </div>
            <form method="POST" action="{{ route('sdg-contributions.store') }}" class="auth-form" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="title" name="title" rows="3" required>{{ old('title') }}</textarea>
                    @error('title')
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="sdg">
                                SDG Number <span style="color: var(--danger);">*</span>
                            </label>
                            <select class="form-control" id="sdg" name="sdg" required>
                                <option value="">Select SDG</option>
                                @for($i = 1; $i <= 17; $i++)
                                    <option value="{{ $i }}" {{ old('sdg') == $i ? 'selected' : '' }}>
                                        SDG {{ $i }}
                                    </option>
                                @endfor
                            </select>
