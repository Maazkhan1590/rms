...
@section('title', 'Conference Activities | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>Conference Activities</h1>
        <p>Browse approved conference activities</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('conference-activities.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search conference activities..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="type" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($activities->pluck('activity_type')->unique()->filter() as $type)
                ...
                <select name="year" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($activities->pluck('year')->unique()->filter()->sortDesc() as $year)
                ...
                <select name="sort" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    <option value="conference" {{ request('sort') == 'conference' ? 'selected' : '' }}>Conference A-Z</option>
                </select>
...
<section class="publications-grid-section">
    <div class="container">
        ...
        <div class="publications-grid-full" id="activities-container">
            @include('conference-activities.partials.activity-card', ['activities' => $activities])
        </div>
        @if($hasMore ?? false)
        <div style="text-align: center; margin-top: 3rem;">
            ...
            <div id="loading-indicator" style="display: none; margin-top: 1rem;">
                ...
                <p style="margin-top: 0.5rem; color: var(--text-light);">Loading more activities...</p>
            </div>
        </div>
        @endif
        @else
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            <i class="fas fa-microphone" style="font-size: 3rem; color: var(--text-secondary); margin-bottom: 1rem;"></i>
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No conference activities available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentOffset = {{ $activities->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            fetch('{{ route("conference-activities.load-more") }}?' + new URLSearchParams(params), {
                headers: {
            ...
            .then(data => {
                if (data.html) {
                    ...
                    if (data.hasMore) {
                        ...
                        allLoadedMsg.innerHTML = '<i class="fas fa-check-circle"></i> All activities loaded';
