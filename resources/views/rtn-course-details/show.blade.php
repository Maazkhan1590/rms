...
@section('title', $course->course_code . ' | RTN Course Detail - RMS')
...
<section class="publication-detail-page">
    <div class="container">
        <div class="publication-detail-card">
            <a href="{{ route('rtn-course-details.index') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; color: #6b7280; text-decoration: none; margin-bottom: 1.5rem; font-weight: 500;">
                <i class="fas fa-arrow-left"></i> Back to RTN Course Details
            </a>

            @auth
                @if($course->status === 'draft' && ($course->submitted_by === auth()->id()))
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        ...
                        <form action="{{ route('rtn-course-details.submit', $course->id) }}" method="POST" style="margin: 0;">
                            @csrf
            ...
            <div class="publication-header">
                <h1 class="publication-title">{{ $course->course_code }} - {{ $course->course_name }}</h1>
                @if($course->submitter)
                <div style="margin-bottom: 1rem;">
                    <strong>Submitted by:</strong> {{ $course->submitter->name }}
                </div>
                @endif
            ...
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-info-circle" style="color: #3b82f6; margin-right: 0.5rem;"></i>Course Details
                </h2>
                <div class="detail-grid">
                    <div class="detail-item">
                        <div class="detail-label">RTN Type</div>
                        <div class="detail-value" style="font-weight: 700;">{{ str_replace('_', ' ', strtoupper($course->rtn_type)) }}</div>
                    </div>
                    @if($course->year)
                    <div class="detail-item">
                        <div class="detail-label">Year</div>
                        <div class="detail-value">{{ $course->year }}</div>
                    </div>
                    @endif
                    @if($course->points_allocated)
                    <div class="detail-item">
                        <div class="detail-label">Points Allocated</div>
                        <div class="detail-value" style="font-weight: 700; color: #059669;">{{ number_format($course->points_allocated, 2) }}</div>
                    </div>
                    @endif
                </div>
            </div>
            ...
            @if($course->evidence_link || $course->evidenceFiles->count() > 0 || $course->evidence_description)
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-paperclip" style="color: #3b82f6; margin-right: 0.5rem;"></i>Evidence & Attachments
                </h2>
                
                @if($course->evidence_description)
                <div style="margin-bottom: 1.5rem; padding: 1rem; background: #f9fafb; border-radius: 8px;">
                    ...
                    <p style="color: #4b5563; margin: 0;">{{ $course->evidence_description }}</p>
                </div>
                @endif

                @if($course->evidence_link)
                <div style="margin-bottom: 1.5rem;">
                    <strong style="color: #374151; display: block; margin-bottom: 0.5rem;">Evidence Link:</strong>
                    <a href="{{ $course->evidence_link }}" target="_blank" style="color: #3b82f6; text-decoration: none; word-break: break-all;">
                        <i class="fas fa-external-link-alt"></i> {{ $course->evidence_link }}
                    </a>
                </div>
                @endif

                @if($course->evidenceFiles && $course->evidenceFiles->count() > 0)
                <table class="evidence-table">
                    ...
                    <tbody>
                        @foreach($course->evidenceFiles as $file)
                        <tr>
            ...
            @if($rtn->submitter || $rtn->approver || isset($rtn->faculty))
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value">{{ $rtn->submitter->name }}</div>
                        @if($rtn->submitted_at)
                    ...
                    <div class="detail-item">
                        ...
                        @endif
                        <div class="detail-value">{{ $rtn->approver->name }}</div>
                        @if($rtn->approved_at)
                        <div style="font-size: 0.85rem; color: #6b7280; margin-top: 0.5rem;">
                            <i class="far fa-calendar"></i> {{ $rtn->approved_at->format('F d, Y') }}
                        </div>
                        @endif
                    </div>
                    @endif

                    @if(isset($rtn->faculty) && $rtn->faculty)
                    <div class="detail-item">
