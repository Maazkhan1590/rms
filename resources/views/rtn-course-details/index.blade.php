...
@section('title', 'RTN Course Details | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>RTN Course Details</h1>
        <p>Browse approved RTN course details</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('rtn-course-details.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search RTN courses..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="rtn_type" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($courses->pluck('rtn_type')->unique()->filter() as $type)
                ...
                <select name="year" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($courses->pluck('year')->unique()->filter()->sortDesc() as $year)
                ...
                <select name="sort" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    <option value="course" {{ request('sort') == 'course' ? 'selected' : '' }}>Course Code A-Z</option>
                </select>
...
<section class="publications-grid-section">
    <div class="container">
        @if($courses->count() > 0)
        <div class="publications-grid-full" id="courses-container">
            @include('rtn-course-details.partials.course-card', ['courses' => $courses])
        </div>
        @if($hasMore ?? false)
        ...
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            ...
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No RTN course details available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentOffset = {{ $courses->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            fetch('{{ route("rtn-course-details.load-more") }}?' + new URLSearchParams(params), {
                headers: {
