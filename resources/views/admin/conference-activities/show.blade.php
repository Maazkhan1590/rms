...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="mb-0">
            <span class="material-icons-outlined" style="vertical-align: middle;">mic</span>
            <span style="vertical-align: middle;">Conference Activity Details</span>
        </h3>
        <div>
            <a href="{{ route('admin.conference-activities.index') }}" class="btn btn-secondary">
    ...
    <div class="card-body">
        <div class="row">
            <div class="col-md-8">
                <h4>{{ $activity->conference }}</h4>
                
                <table class="table table-bordered">
                    <tr>
                        <th width="200">Activity Type</th>
                        <td>{{ ucfirst($activity->activity_type ?? 'N/A') }}</td>
                    </tr>
                    <tr>
                        <th>Country</th>
                    ...
                    <tr>
                        <th>Year</th>
                        <td>{{ $activity->year ?? ($activity->date ? $activity->date->format('Y') : 'N/A') }}</td>
                    </tr>
                    <tr>
                        <th>Status</th>
                        <td>
                            ...
                            @elseif(in_array($activity->status, ['pending_coordinator', 'pending_dean']))
                                <span class="badge badge-warning">{{ ucfirst(str_replace('_', ' ', $activity->status)) }}</span>
                            @elseif($activity->status == 'rejected')
                                <span class="badge badge-danger">Rejected</span>
                            @elseif($activity->status == 'draft')
                                <span class="badge badge-secondary">Draft</span>
                            @else
                                <span class="badge badge-info">{{ ucfirst($activity->status) }}</span>
                            @endif
                        </td>
                    </tr>
                    @if($activity->date)
                    <tr>
                        <th>Date</th>
                        <td>{{ $activity->date->format('F d, Y') }}</td>
                    </tr>
                    @endif
                    @if($activity->submitter)
                    <tr>
                        <th>Submitted By</th>
                        <td>{{ $activity->submitter->name }}</td>
                    </tr>
                    @endif
                    <tr>
                        ...
                        <td>
                            @if($activity->points_allocated)
                                <strong style="color: var(--primary); font-size: 1.2em;">{{ number_format($activity->points_allocated, 2) }}</strong>
                                @if($activity->points_locked)
                    ...
                    <div class="card-body">
                        <table class="table table-bordered">
                            <tr>
                                ...
                                <td>
                                    @if($activity->workflow->status == 'approved')
                                        <span class="badge badge-success">Approved</span>
                                    @elseif($activity->workflow->status == 'pending_coordinator')
                                        <span class="badge badge-warning">Pending Coordinator</span>
                                    @elseif($activity->workflow->status == 'pending_dean')
                                        <span class="badge badge-info">Pending Dean</span>
                                    @else
                                        <span class="badge badge-secondary">{{ ucfirst(str_replace('_', ' ', $activity->workflow->status)) }}</span>
                                    @endif
                                </td>
                            </tr>
                            @if($activity->workflow->assignee)
                            <tr>
                                <th>Assigned To</th>
                                <td>{{ $activity->workflow->assignee->name }}</td>
                            </tr>
                            @endif
                        </table>
...
<div class="modal fade" id="rejectModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{ route('admin.conference-activities.reject', $activity->id) }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title">Reject Conference Activity</h5>
