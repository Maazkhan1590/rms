        ...
        <form action="{{ route('admin.site-contents.store') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('key') is-invalid @enderror" 
                        id="key" name="key" required onchange="updateLabelFromKey();">
                        ...
                        <optgroup label="{{ ucfirst($section) }} Section">
                            @foreach($keys as $keyValue => $keyLabel)
                                <option value="{{ $keyValue }}" {{ old('key') == $keyValue ? 'selected' : '' }} 
                                        data-label="{{ $keyLabel }}" data-section="{{ $section }}" data-type="{{ 
                ...
                @enderror
                <small class="form-text text-muted">Select a predefined key from the list above</small>
            </div>
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-primary">
                    <span class="material-icons-outlined" style="font-size: 16px; vertical-align: middle;">save</span> Create Content
                </button>
