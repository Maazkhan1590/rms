...
@section('title', Str::limit($funding->project_title, 50) . ' | Internal Funding Details - RMS')
...
<section class="publication-detail-page">
    <div class="container">
        <div class="publication-detail-card">
            <a href="{{ route('internal-fundings.index') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; color: #6b7280; text-decoration: none; margin-bottom: 1.5rem; font-weight: 500;">
                <i class="fas fa-arrow-left"></i> Back to Internal Fundings
            </a>

            @auth
                @if($funding->status === 'draft' && ($funding->submitted_by === auth()->id()))
                <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 1rem 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                        ...
                        <form action="{{ route('internal-fundings.submit', $funding->id) }}" method="POST" style="margin: 0;">
                            @csrf
            ...
            <div class="publication-header">
                <h1 class="publication-title">{{ $funding->project_title }}</h1>
                @if($funding->submitter)
                <div style="margin-bottom: 1rem;">
                    <strong>Submitted by:</strong> {{ $funding->submitter->name }}
                </div>
                @endif
            
                @php
                    $displayStatus = $funding->status;
                    $statusColors = [
            ...
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-info-circle" style="color: #3b82f6; margin-right: 0.5rem;"></i>Funding Details
                </h2>
                <div class="detail-grid">
                    @if($funding->funding_source)
                    <div class="detail-item">
                        <div class="detail-label">Funding Source</div>
                        <div class="detail-value">{{ $funding->funding_source }}</div>
                    </div>
                    @endif
                    @if($funding->amount_omr)
                    <div class="detail-item">
                        <div class="detail-label">Amount</div>
                        <div class="detail-value" style="font-weight: 700; color: #059669;">{{ number_format($funding->amount_omr, 2) }} OMR</div>
                    </div>
                    @endif
                    @if($funding->start_date)
                    <div class="detail-item">
                        <div class="detail-label">Start Date</div>
                        <div class="detail-value">{{ $funding->start_date->format('F d, Y') }}</div>
                    </div>
                    @endif
                    @if($funding->end_date)
                    <div class="detail-item">
                        <div class="detail-label">End Date</div>
                        <div class="detail-value">{{ $funding->end_date->format('F d, Y') }}</div>
                    </div>
                    @endif
                    @if($funding->year)
                    <div class="detail-item">
                        <div class="detail-label">Year</div>
                        <div class="detail-value">{{ $funding->year }}</div>
                    </div>
                    @endif
                    @if($funding->points_allocated)
                    <div class="detail-item">
                        <div class="detail-label">Points Allocated</div>
                        <div class="detail-value" style="font-weight: 700; color: #059669;">{{ number_format($funding->points_allocated, 2) }}</div>
                    </div>
                    @endif
                </div>
            </div>
            ...
            @if($funding->evidence_link || $funding->evidenceFiles->count() > 0 || $funding->evidence_description)
            <div style="margin-bottom: 2.5rem;">
                <h2 class="section-title">
                    <i class="fas fa-paperclip" style="color: #3b82f6; margin-right: 0.5rem;"></i>Evidence & Attachments
                </h2>
                
                @if($funding->evidence_description)
                <div style="margin-bottom: 1.5rem; padding: 1rem; background: #f9fafb; border-radius: 8px;">
                    ...
                    <p style="color: #4b5563; margin: 0;">{{ $funding->evidence_description }}</p>
                </div>
                @endif

                @if($funding->evidence_link)
                <div style="margin-bottom: 1.5rem;">
                    <strong style="color: #374151; display: block; margin-bottom: 0.5rem;">Evidence Link:</strong>
                    <a href="{{ $funding->evidence_link }}" target="_blank" style="color: #3b82f6; text-decoration: none; word-break: break-all;">
                        <i class="fas fa-external-link-alt"></i> {{ $funding->evidence_link }}
                    </a>
                </div>
                @endif
