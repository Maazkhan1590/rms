...
@section('title', 'Internal Fundings | Academic Research Portal')

@section('content')
<header class="page-header">
    <div class="container">
        <h1>Internal Fundings</h1>
        <p>Browse approved internal funding records</p>
    </div>
</header>

<section class="publications-filter">
    <div class="container">
        <form action="{{ route('internal-fundings.index') }}" method="GET" class="filter-container">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search internal fundings..." style="flex: 1; min-width: 300px; padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); font-size: 1rem;">
            <div class="filter-options">
                <select name="year" style="padding: 1rem 1.5rem; border: 1px solid var(--border-color); border-radius: var(--border-radius); background: white; font-size: 0.95rem;">
                    ...
                    @foreach($fundings->pluck('year')->unique()->filter()->sortDesc() as $year)
...
<section class="publications-grid-section">
    <div class="container">
        @if($fundings->count() > 0)
        <div class="publications-grid-full" id="fundings-container">
            @include('internal-fundings.partials.funding-card', ['fundings' => $fundings])
        </div>
        @if($hasMore ?? false)
        ...
        <div style="text-align: center; padding: 4rem; background: white; border-radius: 15px;">
            <i class="fas fa-money-bill-wave" style="font-size: 3rem; color: var(--text-secondary); margin-bottom: 1rem;"></i>
            <p style="color: var(--text-secondary); font-size: 1.125rem;">No internal funding records available yet.</p>
        </div>
        @endif
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const fundingsContainer = document.getElementById('fundings-container');
    let currentOffset = {{ $fundings->count() }};
    let isLoading = false;
    ...
    if (loadMoreBtn) {
        loadMoreBtn.addEventListener('click', function() {
            ...
            fetch('{{ route("internal-fundings.load-more") }}?' + new URLSearchParams(params), {
                headers: {
            ...
            .then(data => {
                if (data.html) {
                    ...
                    cards.forEach(card => {
                        fundingsContainer.appendChild(card);
                    });
